# Treść — #312 Oxyfertil 90 (pilot T-136, zaakceptowane `[J 11.09]`)

> 11.09.2026 · research: [`oxyfertil-90-research.md`](oxyfertil-90-research.md) · baza: [`../oxyfertil-90.md`](../oxyfertil-90.md)
> Źródła treści: **[K]** karta AGRII PDF (`/wp-content/uploads/2026/06/agria-karta-produktu-oxyfertil-90.pdf`) · **[K311]** karty Agrobielika 90 (0–3 i 2–8 mm) ·
> **[C]** `CENNIK_PAWEL_2026-08-07.md` · **[F]** `FAKTY_KLIENTA.md` §5 (flota z katalogu). Nic spoza tych źródeł.
> Nazwa WC, adres, kategoria, `_price` — bez zmian. Warstwa renderu: `post_content` (+ `post_excerpt` jako lead pod H1).

## 1. Frazy przypisane i hipoteza

**Frazy karty** (≈ 140 wyszukań/mies., planer): `oxyfertil` 30 · `wapno oxyfertil` 20 · `oxyfertil 90` 10 · `oxyfertil wapno` 10 ·
`wapno tlenkowe oxyfertil` 10 · `oxyfertil 90 cena` 10 · `oxyfertil cena` 10 · `wapno oxyfertil cena` 10 · `oxyfertil dawkowanie` 10 ·
`wapno oxyfertil dawkowanie` 10 · `oxyfertil lhoist` 10. Poza kartą: `wapno tlenkowe 90` (wspólna z #311), `wapno nawozowe tlenkowe` (D1), frazy Lhoist (6 kart).

**Hipoteza:** po zmianie #312 podniesie CTR z 4,79% (15 klik. / 313 wyśw., 28 dni do 07.09) do co najmniej 7% i przejdzie na `oxyfertil 90`
z poz. 4,9 na 1–3, bo w wyniku pojawią się producent i karta PDF, które ma konkurencja nad nami. **Kontrola: dzień wdrożenia + 28 dni.**

## 1a. Mapa: fraza → miejsce na karcie

| fraza (wyszukań/mies.) | gdzie pracuje |
|---|---|
| `oxyfertil 90` 10 · `wapno tlenkowe oxyfertil` 10 | title, H1 (nazwa WC), lead |
| `wapno oxyfertil` 20 · `oxyfertil wapno` 10 · `oxyfertil lhoist` 10 | H2 „Wapno Oxyfertil 90 — wapno tlenkowe od Lhoist…", title („Lhoist"), meta |
| `oxyfertil` 30 · PAA „Jakie jest wapno Oxyfertil?" | FAQ „Jakie jest wapno Oxyfertil 90?" |
| PAA „Co daje wapno tlenkowe?" | FAQ „Co daje wapno tlenkowe Oxyfertil 90?" |
| `oxyfertil zastosowanie` <10 · `oxyfertil dawkowanie` 10 · `wapno oxyfertil dawkowanie` 10 · PAA „Jaka dawka wapna tlenkowego na hektar?" | H2 „Oxyfertil 90 — zastosowanie i dawkowanie", FAQ „Oxyfertil 90 — dawkowanie: ile na hektar?", FAQ o osadach |
| `oxyfertil czy agrobielik` <10 | H2 „Oxyfertil 90 czy Agrobielik 90?" (tabela), FAQ „…czym się różnią?" |
| `oxyfertil 90 cena` 10 · `oxyfertil cena` 10 · `wapno oxyfertil cena` 10 · `oxyfertil big bag` <10 | H2 „Oxyfertil 90 — cena i dostawa w big-bagach", FAQ „Ile kosztuje…", lead (cena) |
| nazwa + dokumenty (karty PDF dystrybutorów w top 5 SERP) | link do karty PDF pod specyfikacją, FAQ „Jakie dokumenty…" |

**Schemat:** `Product` (nazwa = nazwa produktu, nie tytuł strony) + `offers` (bez zmian) + **`FAQPage` z 7 pytań**.

## 2. Meta

| pole | dziś | nowe |
|---|---|---|
| title (47 zn.) | Wapno nawozowe tlenkowe Oxyfertil 90 \| Odkwaszanie \| AGRIA | **Oxyfertil 90 – wapno tlenkowe od Lhoist \| AGRIA** |
| meta description (150 zn.) | …odkaża stawy. Wysoka reaktywność… | **Oxyfertil 90 od Lhoist: wapno tlenkowe min. 90% CaO, kruszone 3–8 mm, w big-bagach 1000 kg. Oczyszczalnie i rolnictwo, dawka 1–3 t/ha. Karta PDF.** |
| focus keyword | wapno nawozowe tlenkowe oxyfertil 90 | **oxyfertil 90** |

## 3. Lead (`post_excerpt`, pod H1 obok zdjęcia)

> Oxyfertil 90 to wapno nawozowe tlenkowe o zawartości min. 90% CaO, produkowane przez Lhoist. Kruszone, frakcja 3–8 mm, w big-bagach
> 1000 kg — z magazynów w Niedomicach, Tarnowie Opolskim i Górażdżach. Służy do higienizacji osadów ściekowych, neutralizacji
> i interwencyjnego odkwaszania gleb, a działa szybko: w 7–14 dni.
>
> **Cena: od 790 zł/t netto w big-bagu — za towar, bez transportu.**

## 4. Treść (`post_content`)

### H2 Wapno Oxyfertil 90 — wapno tlenkowe od Lhoist dla oczyszczalni i rolnictwa

Oxyfertil 90 to wapno tlenkowe o zawartości min. 90% CaO i bardzo wysokiej reaktywności, produkowane przez Lhoist. Przeznaczone jest
do szybkiej neutralizacji kwasowości, higienizacji osadów ściekowych oraz interwencyjnego odkwaszania gleb. Reakcja jest egzotermiczna
i natychmiastowa, a efekt — wzrost pH i sanityzacja — pojawia się w ciągu 7–14 dni.

W ofercie AGRII to drugie, obok Agrobielika 90, wapno tlenkowe o zawartości min. 90% CaO: ten sam standard jakościowy, inny producent.
Oczyszczalnie i duże gospodarstwa mają dzięki temu drugą ścieżkę dostawy — z magazynu AGRII albo bezpośrednio od producenta.
Do przetargów i kontraktów wieloletnich przygotowujemy pełną dokumentację techniczną.

### H2 Oxyfertil 90 — zastosowanie i dawkowanie

- **Higienizacja osadów ściekowych** — 20–40% suchej masy osadu. Spełnia normy temperaturowe i sanitarne.
- **Interwencyjne odkwaszanie gleb** — 1–3 t/ha. Podnosi pH i uzupełnia wapń pokarmowy; działa w 7–14 dni.
- **Neutralizacja kwasowości** — szybka, natychmiastowa reakcja i wzrost pH.
- **Stabilizacja pH przed rekultywacją** — zastosowanie dodatkowe.

Termin i technikę wapnowania pól opisujemy w poradniku [Jak stosować wapno nawozowe](/jak-stosowac-wapno-nawozowe/),
a dawkę pod konkretną glebę policzysz w [kalkulatorze wapnowania](/kalkulator-wapnowania/).

### H2 Oxyfertil 90 czy Agrobielik 90?

Oba produkty mają na kartach tę samą zawartość CaO, zastosowanie, dawkowanie i efekt. Różnią się producentem, frakcją, magazynami
i formą dostawy.

| | **Oxyfertil 90** | [Agrobielik 90, 2–8 mm](/wapno-nawozowe-rolnictwo/agrobielik-90/) | [Agrobielik 90, 0–3 mm](/wapno-nawozowe-rolnictwo/agrobielik-90/) |
|---|---|---|---|
| Zawartość CaO | min. 90% | min. 90% | min. 90% |
| Forma, frakcja | kruszone, 3–8 mm | kruszone, 2–8 mm | sypkie, 0–3 mm |
| Szybkość działania | 7–14 dni | 7–14 dni | 2–4 dni |
| Dawkowanie w rolnictwie | 1–3 t/ha | 1–3 t/ha | 1–3 t/ha |
| Producent | Lhoist | Nordkalk | Nordkalk |
| Magazyny | Niedomice, Tarnów Opolski, Górażdże | Niedomice, Sitkówka | Niedomice, Sitkówka |
| Forma dostawy | big-bag 1000 kg | big-bag 1000 kg, luz 24 t | big-bag 1000 kg, luz 24 t |
| Cena od (zł/t netto, bez transportu) | 790 w big-bagu | 850 luzem | 750 luzem |

### H2 Oxyfertil 90 — specyfikacja techniczna

Tabela 1:1 z karty PDF. Wiersz „Forma dostawy" wraca `[J 11.09]`.

| Parametr | Wartość |
|---|---|
| Zawartość CaO | min. 90% CaO |
| Reaktywność | Bardzo wysoka |
| Typ reakcji | Egzotermiczna (natychmiastowa) |
| Forma fizyczna | Kruszone |
| Frakcja | 3–8 mm |
| Zastosowanie | Higienizacja osadów, neutralizacja, odkwaszanie interwencyjne |
| Efekt zastosowania | Wzrost pH, sanityzacja. Uzupełnienie wapnia pokarmowego |
| Dawkowanie | 20–40% suchej masy osadu, 1–3 t/ha (rolnictwo) |
| Szybkość działania | Szybkie (7–14 dni) |
| Dodatkowe zastosowanie | Stabilizacja pH przed rekultywacją |
| Segment | Oczyszczalnie, rolnictwo, rybactwo, hurtownie |
| **Forma dostawy** | **Big-bag 1000 kg** |
| Magazyn | Niedomice (33-132), Tarnów Opolski (46-050), Górażdże (47-316) |
| Producent | Lhoist |
| Dostępność | Cały rok |

Pod tabelą: **[Pobierz kartę produktu Oxyfertil 90 (PDF)](/wp-content/uploads/2026/06/agria-karta-produktu-oxyfertil-90.pdf)**

### H2 Oxyfertil 90 — cena i dostawa w big-bagach

**Oxyfertil 90** kosztuje **od 790 zł/t netto** w big-bagach od 1 tony.

Podane kwoty dotyczą samego towaru, bez transportu. Ceny orientacyjne, netto, nie stanowią oferty handlowej w rozumieniu Kodeksu cywilnego.

Towar wysyłamy z magazynu AGRII w Niedomicach albo bezpośrednio od producenta — z Tarnowa Opolskiego lub Górażdży. Dowozimy własnym
transportem, samochodami od 3 do 24 t; koszt dostawy pod Twój adres podamy w wycenie.

### H2 Najczęściej zadawane pytania

**H3 Jakie jest wapno Oxyfertil 90?**
To wapno nawozowe tlenkowe produkowane przez Lhoist: min. 90% CaO, forma kruszona, frakcja 3–8 mm, bardzo wysoka reaktywność
i natychmiastowa reakcja egzotermiczna. Stosuje się je tam, gdzie pH trzeba podnieść szybko — w osadach ściekowych i przy interwencyjnym odkwaszaniu gleb.

**H3 Co daje wapno tlenkowe Oxyfertil 90?**
Podnosi pH i sanityzuje — w osadach ściekowych i w glebie. Karta produktu podaje trzy efekty: wzrost pH, sanityzację
i uzupełnienie wapnia pokarmowego. Neutralizuje kwasowość szybko, bo reakcja jest egzotermiczna i natychmiastowa; efekt pojawia się w 7–14 dni.

**H3 Oxyfertil 90 — dawkowanie: ile na hektar?**
W rolnictwie karta produktu podaje 1–3 t/ha. To wapno do interwencyjnego odkwaszania — działa w 7–14 dni. Dawkę pod swoją glebę
policzysz w [kalkulatorze wapnowania](/kalkulator-wapnowania/).

**H3 Jak dawkuje się Oxyfertil 90 przy higienizacji osadów ściekowych?**
20–40% suchej masy osadu. Oxyfertil 90 podnosi pH i sanityzuje osad; spełnia normy temperaturowe i sanitarne higienizacji.

**H3 Oxyfertil 90 czy Agrobielik 90 — czym się różnią?**
Zawartość CaO (min. 90%), zastosowanie, dawkowanie i szybkość działania są na kartach obu produktów takie same. Oxyfertil 90 produkuje
Lhoist, Agrobielik 90 — Nordkalk. Oxyfertil 90 ma frakcję 3–8 mm i jest dostępny w big-bagach, Agrobielik 90 — 2–8 mm lub 0–3 mm,
w big-bagach i luzem. Zestawienie z cenami znajdziesz wyżej na tej stronie.

**H3 Ile kosztuje Oxyfertil 90 i czy dowozicie?**
Od 790 zł/t netto w big-bagu — za towar, bez transportu. Dowozimy własnym transportem z magazynu AGRII w Niedomicach albo prosto
od producenta. Napisz, ile big-bagów potrzebujesz i dokąd, a koszt dostawy podamy w wycenie.

**H3 Jakie dokumenty są dostępne dla Oxyfertilu 90?**
Karta produktu jest do pobrania pod specyfikacją. Dla oczyszczalni i odbiorców kontraktowych przygotowujemy pełną dokumentację
techniczną pod przetargi i kontrakty wieloletnie — zapytaj handlowca.

### H2 Zapytaj o ofertę, zamów próbkę — *bez zmian*
Skontaktuj się z naszym zespołem handlowym, aby uzyskać indywidualną wycenę, zamówić próbkę produktu lub omówić warunki dostawy.
Agria — 37 lat doświadczenia na rynku nawozów wapniowych.

**H3 Poradniki techniczne** — *bez zmian* (linki do `/wapnowanie-gleby/` i `/jak-stosowac-wapno-nawozowe/`)

## 5. Schemat

- `FAQPage` — 7 pytań z sekcji FAQ, treść 1:1. Generowane przy wdrożeniu (propozycja: ten sam filtr `rank_math/json_ld` w `seo-head.php`, który dokłada
  `offers`, czyta pary H3 → odpowiedź spod „Najczęściej zadawane pytania" — jedna zmiana dla wszystkich kart; wymaga Twojej zgody jako zmiana kodu).
- `Product` / `offers` — bez zmian (790 PLN, `TNE`).

## 6. Decyzje przy akcepcie

1. ~~Wiersz „Forma dostawy"~~ — **rozstrzygnięte `[J 11.09]`: wraca 1:1 z karty PDF**, bez SKU, MOQ i słowa „minimum" (Paweł ograniczał minimum, nie formy dostawy).
2. ~~„Góraźdźce" w tabeli~~ — **rozstrzygnięte `[J 11.09]`: poprawiamy na stronie na „Górażdże (47-316)"** (literówka karty PDF, nie parametr).

## 7. Co znika w stosunku do dzisiejszej karty

~100% reaktywności · pH osadów „powyżej 12" (treść i FAQ) · +0,5–2 pH w 2–4 tygodnie · „widoczne już po 2 tygodniach" · stawy: odkażanie wody i dna, 40–60 kg/1000 m³,
mineralizacja mułu (treść, tabela, FAQ) · „gleby bardzo kwaśne (pH <5), intensywna hodowla ryb, ścieki przemysłowe" (tabela) · wiersz „Czas działania" z literówką ·
„karty charakterystyki, certyfikaty" · sekcja „Optymalizacja procesów, wymierne korzyści dla biznesu" · „Bardzo szybkie" → „Szybkie (7–14 dni)" jak w karcie.
