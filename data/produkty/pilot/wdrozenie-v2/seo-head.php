...
/*
 * ─────────────────────────────────────────────────────────────────────────────
 * Karty po modernizacji: FAQPage + nazwa produktu w wezle Product (T-136, 11.09)
 * ─────────────────────────────────────────────────────────────────────────────
 *
 * FAQPage powstaje z sekcji "Najczesciej zadawane pytania" w post_content
 * (pary H3 -> odpowiedz), wiec pytania w schemacie sa zawsze tymi, ktore widzi
 * czlowiek — ta sama zasada co przy offers: zgodnosc z konstrukcji, nie z pamieci.
 *
 * Nazwa produktu: Rank Math wstawia do Product tytul SEO strony ("... | AGRIA").
 * Nazwa produktu to nazwa z WooCommerce.
 *
 * LISTA ID rosnie karta po karcie: karta trafia tu dopiero po modernizacji
 * i akcepcie Janka. Pozostale karty maja jeszcze FAQ z tekstu DescWriter,
 * ktorego nie wypuszczamy do schematu.
 */

if ( ! defined( 'AGRIA_KARTY_SCHEMA_V2' ) ) {
	define( 'AGRIA_KARTY_SCHEMA_V2', array( 312 ) );
}

if ( ! function_exists( 'agria_faq_z_tresci' ) ) {
	/**
	 * Pary pytanie-odpowiedz z sekcji FAQ karty.
	 *
	 * @param string $tresc post_content karty.
	 * @return array Wezly Question.
	 */
	function agria_faq_z_tresci( string $tresc ): array {
		$start = strpos( $tresc, 'id="najczciejzadawanepytania"' );
		if ( false === $start ) {
			return array();
		}
		$sekcja = substr( $tresc, $start );
		$koniec = strpos( $sekcja, '<h2', 10 );
		if ( false !== $koniec ) {
			$sekcja = substr( $sekcja, 0, $koniec );
		}
		preg_match_all( '#<h3[^>]*>(.*?)</h3>(.*?)(?=<h3|$)#s', $sekcja, $trafienia, PREG_SET_ORDER );

		$pytania = array();
		foreach ( $trafienia as $para ) {
			$pytanie    = trim( html_entity_decode( wp_strip_all_tags( $para[1] ), ENT_QUOTES, 'UTF-8' ) );
			$odpowiedz  = trim( preg_replace( '/\s+/u', ' ', html_entity_decode( wp_strip_all_tags( $para[2] ), ENT_QUOTES, 'UTF-8' ) ) );
...
if ( ! function_exists( 'agria_schema_karty_v2' ) ) {
	/**
	 * FAQPage i nazwa produktu dla kart z listy AGRIA_KARTY_SCHEMA_V2.
	 *
	 * @param array $data Graf JSON-LD Rank Matha.
	 * @return array
	 */
	function agria_schema_karty_v2( $data ) {
		if ( ! is_array( $data ) || ! is_singular( 'product' ) ) {
			return $data;
		}
		$post = get_queried_object();
		if ( ! $post instanceof WP_Post || ! in_array( (int) $post->ID, AGRIA_KARTY_SCHEMA_V2, true ) ) {
			return $data;
		}

		foreach ( $data as $klucz => $wezel ) {
			if ( is_array( $wezel ) && in_array( 'Product', (array) ( $wezel['@type'] ?? '' ), true ) ) {
				$data[ $klucz ]['name'] = $post->post_title;
			}
		}

		$faq = agria_faq_z_tresci( (string) $post->post_content );
		if ( count( $faq ) >= 2 ) {
			$data['agriaFaqPage'] = array(
				'@type'      => 'FAQPage',
	...
	add_filter( 'rank_math/json_ld', 'agria_schema_karty_v2', 100, 1 );
}
