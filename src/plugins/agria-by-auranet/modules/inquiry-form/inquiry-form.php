...
class Agria_Inquiry_Form {
    ...
    public function inquiry_column_data( string $column, int $post_id ): void {
        ...
        switch ( $column ) {
            case 'inq_type':
                $typ = $meta( '_inquiry_type' );
                echo 'callback' === $typ ? '📞 Kontakt' : ( 'probka' === $typ ? '📦 Próbka' : '📋 Oferta' );
                break;
            case 'inq_product':
    ...
    public function render_shortcode( $atts ): string {
        ...
        /*
         * Tryb "callback" (T-059, 2026-09-07).
         *
         * DLACZEGO: na landingach ruchu platnego intencja brzmi "cena za tone",
         * nie "dobierz mi produkt", a formularz kazal wybrac produkt z dwudziestu
         * opcji. Do tego 58,6% budzetu Ads wychodzi, gdy biuro jest zamkniete
         * (pn-pt 8-16), wiec formularz jest wtedy jedyna sciezka kontaktu —
         * i jedyna, ktora realnie dziala: 37 rozpoczec, 32 wyslania, 86%,
         * przy 3 klknieciach w telefon w tym samym kwartale.
         *
         * W tym trybie zostaja trzy pola: imie, telefon, zgoda. Wybor produktu,
         * forma dostawy, ilosc, e-mail, firma, miejscowosc i uwagi znikaja —
         * i z formularza, i z walidacji po stronie serwera.
         */
        $callback = ( 'callback' === $atts['mode'] );

        if ( '' === $atts['title'] ) {
            $atts['title'] = $callback ? 'Zostaw numer, oddzwonimy' : 'Zapytaj o ofertę lub zamów próbkę';
        }
        if ( '' === $atts['submit'] ) {
            $atts['submit'] = $callback ? 'Proszę o kontakt' : 'Wyślij zapytanie';
        }
        ...
        <div class="agria-inquiry-wrap" id="<?php echo esc_attr( $form_id ); ?>">
            ...
            <form class="agria-inquiry-form" novalidate>
                ...
                <?php if ( $callback ) : ?>
                    <input type="hidden" name="inquiry_mode" value="callback">
                    <input type="hidden" name="inquiry_type" value="callback">
                    <input type="hidden" name="product_name" value="<?php echo esc_attr( $ctx['name'] ?: 'Prośba o kontakt' ); ?>">
                <?php endif; ?>
                ...
                <div class="agria-field agria-field-checkbox">
                    <label class="agria-checkbox-label">
                        ...
                        <a href="/rodo/" target="_blank" rel="noopener">Polityka prywatności</a>.</span>
                    </label>
                </div>
                ...
                <div class="agria-msg agria-msg-error" style="display:none;" role="alert">
                    ...
                    <div><strong>Błąd wysyłania.</strong> Spróbuj ponownie lub zadzwoń: <a href="tel:+48664393062">+48 664 393 062</a></div>
                </div>
    ...
    public function handle_submit(): void {
        ...
        // Tryb callback (T-059): egzekwujemy tylko to, co formularz pokazuje.
        // Wymaganie e-maila i produktu przy trzech widocznych polach dawaloby
        // blad, ktorego uzytkownik nie ma jak naprawic.
        $callback = ( 'callback' === sanitize_text_field( $_POST['inquiry_mode'] ?? '' ) );

        if ( $callback && empty( $d['product_name'] ) ) {
            $d['product_name'] = 'Prośba o kontakt';
        }

        if ( empty( $d['full_name'] ) || empty( $d['phone'] ) ) {
            wp_send_json_error( [ 'message' => 'Wypełnij wymagane pola.' ] );
        }
        if ( ! $callback && ( empty( $d['email'] ) || empty( $d['product_name'] ) ) ) {
            wp_send_json_error( [ 'message' => 'Wypełnij wymagane pola.' ] );
        }
    ...
    private function save_inquiry( array $d ): int {
        $type_label = 'callback' === $d['inquiry_type'] ? 'Kontakt' : ( 'probka' === $d['inquiry_type'] ? 'Próbka' : 'Oferta' );
    ...
    private function send_handler_email( array $d, int $inquiry_id ): bool {
        ...
        $type_label = 'callback' === $d['inquiry_type'] ? 'Prośba o kontakt' : ( 'probka' === $d['inquiry_type'] ? 'Zamówienie próbki' : 'Zapytanie ofertowe' );
    ...
    private function send_client_email( array $d ): bool {
        ...
        <div style="max-width:560px;margin:20px auto;background:#fff;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
            ...
            <div style="padding:28px;">
                ...
                <p style="margin:0 0 4px;font-size:14px;color:#333;">📞 <a href="tel:+48664393062" style="color:#2d7a2d;text-decoration:none;font-weight:600;">+48 664 393 062</a></p>
